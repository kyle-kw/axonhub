...
import { useXaiDeviceFlow } from '../hooks/use-xai-device-flow';

interface XaiDeviceFlowProps {
  onSuccess: (credentials: string) => void;
  onError?: (error: string) => void;
  existingCredentials?: string;
}

export function XaiDeviceFlow({ onSuccess, onError, existingCredentials }: XaiDeviceFlowProps) {
  const { t } = useTranslation();
  const hasExistingCredentials = existingCredentials && existingCredentials.trim().length > 0;
  const deviceFlow = useXaiDeviceFlow({
    onSuccess,
  });
        ...
        <div className='rounded-md border border-green-500/50 bg-green-50/10 p-3'>
          <div className='flex items-center gap-2 text-green-600'>
            ...
            <span className='font-medium'>
              {t('channels.dialogs.xai.messages.alreadyConnected')}
            </span>
          </div>
          <p className='text-muted-foreground mt-1 text-xs'>
            {t('channels.dialogs.xai.messages.credentialsStored')}
          </p>
          <Button type='button' onClick={handleReconnect} variant='outline' size='sm' className='mt-2'>
            <RefreshCw className='mr-2 h-4 w-4' />
            {t('channels.dialogs.xai.buttons.reauthenticate')}
          </Button>
        </div>
      </div>
    );
    ...
    return (
      <div className='mt-3 space-y-2'>
        <div className='rounded-md border border-green-500 p-3'>
          <div className='flex items-center gap-2 text-green-600'>
            <CheckCircle2 className='h-5 w-5' />
            <span className='font-medium'>{t('channels.dialogs.xai.messages.authSuccess')}</span>
          </div>
          <p className='text-muted-foreground mt-1 text-xs'>
            {t('channels.dialogs.xai.messages.credentialsImported')}
          </p>
        </div>
      </div>
    );
      ...
      <div className='mt-3 space-y-2'>
        <div className='rounded-md border p-3'>
          <div className='flex items-center gap-2'>
            ...
            <span className='font-medium'>{t('channels.dialogs.xai.messages.waitingForAuth')}</span>
          </div>

          <div className='mt-3 space-y-2'>
            <FormLabel className='text-sm font-medium'>{t('channels.dialogs.xai.labels.userCode')}</FormLabel>
            <div className='flex items-center gap-2'>
              <div className='flex-1 bg-muted p-3 rounded-md text-center'>
                <span className='text-2xl font-mono font-bold tracking-wider'>{deviceFlow.userCode}</span>
              </div>
          ...
          <div className='mt-3 space-y-2'>
            <Button type='button' variant='secondary' onClick={handleOpenVerification} className='w-full'>
              <ExternalLink className='mr-2 h-4 w-4' />
              {t('channels.dialogs.xai.buttons.openVerification')}
            </Button>
            <p className='text-xs text-center text-muted-foreground'>{deviceFlow.verificationUri}</p>
          </div>

          <div className='bg-muted/50 p-3 rounded-md mt-3'>
            <ol className='text-sm space-y-1 list-decimal list-inside text-muted-foreground'>
              <li>{t('xai_device.step_1')}</li>
              <li>{t('xai_device.step_2')}</li>
              <li>{t('xai_device.step_3')}</li>
              <li>{t('xai_device.step_4')}</li>
            </ol>
          </div>
  ...
  return (
    <div className='mt-3 space-y-2'>
      <div className='rounded-md border p-3'>
        <p className='text-muted-foreground mb-2 text-xs'>{t('channels.dialogs.xai.messages.oauthHint')}</p>
        <Button type='button' variant='secondary' onClick={deviceFlow.start} disabled={deviceFlow.isPolling}>
          {deviceFlow.isPolling
            ...
            : t('channels.dialogs.xai.buttons.startOAuth')}
        </Button>
      </div>
    </div>
  );
